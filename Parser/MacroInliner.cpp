...
void Aergia::Parser::MacroInliner::processLoop( IOContext& context, std::wstring loopContent, IContext* currentContext, std::vector<IObject*> const& collection, std::wstring variableName, int& line )
{
	...
	for (auto const var : collection)
	{
		...
		processMacros( localIOContext, &localContext, line );
		context._output.append( outBuffer );
		context._output.append( context._policy.endLine() );
	}
}

void Aergia::Parser::MacroInliner::processMacros( IOContext& context, IContext* currentContext, int& line )
{
	...
	for (size_t i = 0ULL; i < context._input.size(); ++i)
	{
		...
		{
			line++;
			context._output += context._input[i];
			charProcessed = true;
		}
		if (!charProcessed && loops.size() > loopIndex)
		{
			if (i == loops.at( loopIndex )._positionInInputString)
			{
				...
				else
				{
					...
					if (collection)
						processLoop( context, loop._foreachBody, currentContext, *collection, loop._variableName,line );
					else
					{
		...
		if (!charProcessed && calls.size() > callIndex)
		{
			if (i == calls.at( callIndex )._positionInInputString)
			{
				...
				if (object == DataStructures::Null::get())
					context._errorStream << context._policy.warnCallChainFailed( calls.at( callIndex )._conent, line );
...
{
	context._output += L"__anonymous_" + std::to_wstring( lineNumber ) + L"__ = " + contents + L";";
}
...
void Aergia::Parser::MacroInliner::processText( std::wstring const& input, std::wstring & output, std::wostream & errorStream, InliningPolicy const& policy )
{
	...
	int l = 0;
	processMacros( context, &_defaultContext, l );
}
