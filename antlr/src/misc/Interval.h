...
namespace misc {
  ...
  size_t numericToSymbol(ssize_t v) noexcept;
  ssize_t symbolToNumeric(size_t v) noexcept;
  ...
  public:
    ...
    Interval() noexcept;
    explicit Interval(size_t a_, size_t b_) noexcept; // For unsigned -> signed mappings.
    Interval(ssize_t a_, ssize_t b_) noexcept;
    ...
    bool startsBeforeDisjoint(const Interval &other) const noexcept;

    /// <summary>
    /// Does this start at or before other? Nondisjoint </summary>
    bool startsBeforeNonDisjoint(const Interval &other) const noexcept;

    /// <summary>
    /// Does this.a start after other.b? May or may not be disjoint </summary>
    bool startsAfter(const Interval &other) const noexcept;

    /// <summary>
    /// Does this start completely after other? Disjoint </summary>
    bool startsAfterDisjoint(const Interval &other) const noexcept;

    /// <summary>
    /// Does this start after other? NonDisjoint </summary>
    bool startsAfterNonDisjoint(const Interval &other) const noexcept;

    /// <summary>
    /// Are both ranges disjoint? I.e., no overlap? </summary>
    bool disjoint(const Interval &other) const noexcept;

    /// <summary>
    /// Are two intervals adjacent such as 0..41 and 42..42? </summary>
    bool adjacent(const Interval &other) const noexcept;

    bool properlyContains(const Interval &other) const noexcept;
    ...
    Interval intersection(const Interval &other) const noexcept;
