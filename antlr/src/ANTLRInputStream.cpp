...
void ANTLRInputStream::reset() noexcept {
	p = 0;
}
...
size_t ANTLRInputStream::LA( ssize_t i ) {
	...
	if ((position + i - 1LL) >= static_cast<ssize_t>(_data.size())) {
		return IntStream::EOF;
	}

	return _data[static_cast<size_t>((position + i - 1LL))];
}
...
size_t ANTLRInputStream::index() const noexcept {
	return p;
}

size_t ANTLRInputStream::size() const noexcept {
	return _data.size();
}

// Mark/release do nothing. We have entire buffer.
ssize_t ANTLRInputStream::mark() noexcept {
	return -1;
}

void ANTLRInputStream::release( ssize_t /* marker */ ) noexcept {
}
...
std::string ANTLRInputStream::getText( const Interval& interval ) {
	...
	auto const start = interval.a;
	auto  stop = interval.b;
	...
	auto const count = stop - start + 1LL;
	if (start >= _data.size()) {
		return "";
	}
...
void ANTLRInputStream::InitializeInstanceFields() noexcept {
	p = 0;
}
