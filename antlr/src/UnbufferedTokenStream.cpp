...
TokenSource* UnbufferedTokenStream::getTokenSource() const noexcept
{
	return _tokenSource;
}
...
std::string UnbufferedTokenStream::getText( gsl::not_null<RuleContext*> ctx )
{
...
/// </summary>
ssize_t UnbufferedTokenStream::mark() noexcept
{
	if (_numMarkers == 0) {
		_lastTokenBufferStart = _lastToken;
	}

	int const mark = -_numMarkers - 1;
	_numMarkers++;
	return mark;
}
...
{
	ssize_t const expectedMark = -_numMarkers;
...
{
	size_t const bufferStartIndex = getBufferStartIndex();
	size_t const bufferStopIndex = bufferStartIndex + _tokens.size() - 1;

	size_t const start = interval.a;
	size_t const stop = interval.b;
	...
	size_t const a = start - bufferStartIndex;
	size_t const b = stop - bufferStartIndex;

	std::stringstream ss;
	for (size_t i = a; i <= b; i++) {
		gsl::not_null<Token*> t = _tokens[i].get();
		if (i > 0)
			ss << ", ";
		ss << t->getText();
	}
...
size_t UnbufferedTokenStream::getBufferStartIndex() const noexcept
{
	return _currentTokenIndex - _p;
}

void UnbufferedTokenStream::InitializeInstanceFields() noexcept
{
	_p = 0;
	_numMarkers = 0;
