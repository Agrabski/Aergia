...
struct AltAndContextConfigHasher
{
	 ...
	 */
	size_t operator () ( ATNConfig* o ) const noexcept {
...
struct AltAndContextConfigComparer {
	bool operator()( gsl::not_null<ATNConfig*> a, gsl::not_null< ATNConfig*> b ) const
	{
		if (a == b) 
		{
			return true;
...
bool PredictionModeClass::hasConfigInRuleStopState( ATNConfigSet* configs ) noexcept {
...
bool PredictionModeClass::allConfigsInRuleStopStates( ATNConfigSet* configs ) noexcept {
	for (auto& config : configs->configs) {
...
bool PredictionModeClass::allSubsetsConflict( const std::vector<antlrcpp::BitSet>& altsets ) noexcept {
	return !hasNonConflictingAltSet( altsets );
}

bool PredictionModeClass::hasNonConflictingAltSet( const std::vector<antlrcpp::BitSet>& altsets ) noexcept
{
...
bool PredictionModeClass::hasConflictingAltSet( const std::vector<antlrcpp::BitSet>& altsets ) noexcept
{
	for (antlrcpp::BitSet const alts : altsets) {
		if (alts.count() > 1) {
			return true;
		}
	}
...
{
	antlrcpp::BitSet const all = getAlts( altsets );
	if (all.count() == 1) {
...
antlrcpp::BitSet PredictionModeClass::getAlts( const std::vector<antlrcpp::BitSet>& altsets ) noexcept
{
	antlrcpp::BitSet all;
	for (antlrcpp::BitSet const alts : altsets) {
		all |= alts;
...
antlrcpp::BitSet PredictionModeClass::getAlts( gsl::not_null<ATNConfigSet*>configs )
{
