...
void LexerATNSimulator::SimState::reset() noexcept {
	index = INVALID_INDEX;
	line = 0;
...
void LexerATNSimulator::SimState::InitializeInstanceFields() noexcept {
	index = INVALID_INDEX;
	line = 0;
	charPos = INVALID_INDEX;
}
...
void LexerATNSimulator::copyState( LexerATNSimulator* simulator ) noexcept {
...
size_t LexerATNSimulator::match( gsl::not_null< CharStream*> input, size_t mode ) {
	match_calls++;
	_mode = mode;
	ssize_t const mark = input->mark();
	...
	_prevAccept.reset();
	const dfa::DFA& dfa = gsl::at( _decisionToDFA, mode );
	if (dfa.s0 == nullptr) {
...
void LexerATNSimulator::reset() noexcept
{
	_prevAccept.reset();
	_startIndex = 0;
	_line = 1;
...
{
	size_t const size = _decisionToDFA.size();
...
size_t LexerATNSimulator::matchATN( CharStream* input ) {
	ATNState* startState = atn.modeToStartState.at( _mode );

	std::unique_ptr<ATNConfigSet> s0_closure = computeStartState( input, startState );

	bool const suppressEdge = s0_closure->hasSemanticContext;
	s0_closure->hasSemanticContext = false;

	dfa::DFAState* next = addDFAState( s0_closure.release() );
	if (!suppressEdge) {
		gsl::at( _decisionToDFA, _mode ).s0 = next;
	}
...
void LexerATNSimulator::getReachableConfigSet( gsl::not_null< CharStream*> input, gsl::not_null< ATNConfigSet*> closure_, gsl::not_null< ATNConfigSet*> reach, size_t t ) {
	// this is used to skip processing for configs which have a lower priority
	// than a config that already reached an accept state for the same rule
	size_t skipAlt = ATN::INVALID_ALT_NUMBER;

	for (auto c : closure_->configs) {
		bool const currentAltReachedAcceptState = c->alt == skipAlt;
		...
		size_t const n = c->state->transitions.size();
		for (size_t ti = 0; ti < n; ti++) { // for each transition
			gsl::not_null<Transition*> trans = gsl::at(c->state->transitions,ti);
			...
			if (target != nullptr) {
				...
				bool const treatEofAsEpsilon = t == Token::EOF;
...
atn::ATNState* LexerATNSimulator::getReachableTarget( gsl::not_null<Transition*> trans, size_t t ) {
...
Ref<LexerATNConfig> LexerATNSimulator::getEpsilonTarget( CharStream* input, const Ref<LexerATNConfig>& config, Transition* t,
	...
	case Transition::RULE: {
		gsl::not_null<RuleTransition *> const ruleTransition = static_cast<RuleTransition*>(t);
...
bool LexerATNSimulator::evaluatePredicate( CharStream* input, size_t ruleIndex, size_t predIndex, bool speculative ) 
{
	...
	size_t const savedCharPositionInLine = _charPositionInLine;
	size_t const savedLine = _line;
	size_t const index = input->index();
	ssize_t const marker = input->mark();
...
void LexerATNSimulator::captureSimState( CharStream* input, dfa::DFAState* dfaState ) noexcept {
...
size_t LexerATNSimulator::getLine() const noexcept {
	return _line;
}

void LexerATNSimulator::setLine( size_t line ) noexcept {
	_line = line;
}

size_t LexerATNSimulator::getCharPositionInLine() noexcept {
	return _charPositionInLine;
}

void LexerATNSimulator::setCharPositionInLine( size_t charPositionInLine ) noexcept {
	_charPositionInLine = charPositionInLine;
}

void LexerATNSimulator::consume( CharStream* input ) {
	size_t const curChar = input->LA( 1 );
	if (curChar == '\n') {
		_line++;
		_charPositionInLine = 0;
	}
...
void LexerATNSimulator::InitializeInstanceFields() noexcept
{
	_startIndex = 0;
	_line = 1;
	_charPositionInLine = 0;
