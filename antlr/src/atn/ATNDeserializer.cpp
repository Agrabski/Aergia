...
ATN ATNDeserializer::deserialize( const std::vector<uint16_t>& input ) {
	...
	auto const nstates = data[p++];
	for (size_t i = 0; i < nstates; i++) {
		auto const stype = data[p++];
	...
	for (int i = 0; i < nedges; i++) {
		size_t src = data[p];
		size_t trg = data[p + 1LL];
		size_t ttype = data[p + 2LL];
		size_t arg1 = data[p + 3LL];
		size_t arg2 = data[p + 4LL];
		size_t arg3 = data[p + 5LL];
