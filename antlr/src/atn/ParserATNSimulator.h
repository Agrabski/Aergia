...
namespace antlr4 {
	namespace atn {
		...
		public:
			...
			virtual std::string getLookaheadName(gsl::not_null<TokenStream*> input );
		...
		protected:
			...
			///    conflict + preds
			/// </summary>
			virtual size_t execATN( dfa::DFA& dfa, dfa::DFAState* s0, gsl::not_null<TokenStream*>input, size_t startIndex,
			...
			virtual Ref<ATNConfig> predTransition(Ref< ATNConfig> const& config, gsl::not_null< PredicateTransition*> pt, bool collectPredicates,
				bool inContext, bool fullCtx );

			virtual Ref<ATNConfig> ruleTransition( Ref<ATNConfig> const& config, gsl::not_null<RuleTransition*> t );
			...
			static size_t getUniqueAlt( ATNConfigSet* configs ) noexcept;
