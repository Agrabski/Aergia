...
void ParserATNSimulator::reset()noexcept {
}
...
size_t ParserATNSimulator::adaptivePredict( TokenStream* input, size_t decision, ParserRuleContext* outerContext ) {
	...
	ssize_t const m = input->mark();
	size_t const index = _startIndex;
	...
	if (s0 == nullptr) {
		bool const fullCtx = false;
		std::unique_ptr<ATNConfigSet> s0_closure = computeStartState( dfa.atnStartState, &ParserRuleContext::EMPTY, fullCtx );
...
size_t ParserATNSimulator::execATN( dfa::DFA& dfa, dfa::DFAState* s0, gsl::not_null<TokenStream*> input, size_t startIndex,
	...
	while (true) { // while more work
		...
		if (D->requiresFullContext && _mode != PredictionMode::SLL) {
			...
			if (D->predicates.size() != 0) {
				...
				size_t const conflictIndex = input->index();
		...
		if (D->isAcceptState) {
			...
			size_t const stopIndex = input->index();
			input->seek( startIndex );
...
void ParserATNSimulator::predicateDFAState( dfa::DFAState* dfaState, DecisionState* decisionState ) {
	...
	size_t const nalts = decisionState->transitions.size();
...
std::vector<dfa::DFAState::PredPrediction*> ParserATNSimulator::getPredicatePredictions( const antlrcpp::BitSet& ambigAlts,
	...
	bool containsPredicate = std::find_if( altToPred.begin(), altToPred.end(), []( Ref<SemanticContext> const context ) noexcept {
...
Ref<ATNConfig> ParserATNSimulator::predTransition(Ref< ATNConfig> const& config, gsl::not_null< PredicateTransition*> pt,
	bool collectPredicates, bool inContext, bool fullCtx ) {
#if DEBUG_DFA == 1
...
Ref<ATNConfig> ParserATNSimulator::ruleTransition( Ref<ATNConfig> const& config, gsl::not_null<RuleTransition*> t ) {
#if DEBUG_DFA == 1
...
std::string ParserATNSimulator::getLookaheadName(gsl::not_null<TokenStream*> input ) {
...
size_t ParserATNSimulator::getUniqueAlt( ATNConfigSet* configs ) noexcept {
...
void ParserATNSimulator::setPredictionMode( PredictionMode newMode ) noexcept {
	_mode = newMode;
}

atn::PredictionMode ParserATNSimulator::getPredictionMode() noexcept {
	return _mode;
}

Parser* ParserATNSimulator::getParser() noexcept {
	return parser;
}

bool ParserATNSimulator::getLrLoopSetting() {
	char* const var = std::getenv( "TURN_OFF_LR_LOOP_ENTRY_BRANCH_OPT" );
	if (var == nullptr)
		return false;
...
void ParserATNSimulator::InitializeInstanceFields() noexcept {
