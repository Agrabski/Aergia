...
BufferedTokenStream::BufferedTokenStream( TokenSource* tokenSource ) noexcept : _tokenSource( tokenSource )
{
	InitializeInstanceFields();
}

TokenSource* BufferedTokenStream::getTokenSource() const noexcept
{
	return _tokenSource;
}
...
ssize_t BufferedTokenStream::mark() noexcept
{
	return 0;
}

void BufferedTokenStream::release( ssize_t /*marker*/ ) noexcept
{
...
size_t BufferedTokenStream::size() const noexcept
{
	return _tokens.size();
}
...
void BufferedTokenStream::setTokenSource( TokenSource* tokenSource )noexcept
{
...
std::vector<Token*> BufferedTokenStream::getHiddenTokensToRight( size_t tokenIndex, ssize_t channel )
{
	...
	size_t const from = tokenIndex + 1LL;
	size_t to = from;
...
std::vector<Token*> BufferedTokenStream::getHiddenTokensToLeft( size_t tokenIndex, ssize_t channel ) 
{
	...
	if (prevOnChannel == tokenIndex - 1LL) {
		return { };
	}
	// if none onchannel to left, prevOnChannel=-1 then from=0
	size_t const from = static_cast<size_t>(prevOnChannel + 1);
	size_t const to = tokenIndex - 1;
...
std::vector<Token*> BufferedTokenStream::filterForChannel( size_t from, size_t to, ssize_t channel )
{
	...
	for (size_t i = from; i <= to; i++) {
		...
		if (channel == -1) {
			...
			if (static_cast<ssize_t>(t->getChannel()) == channel) {
				hidden.push_back( t );
			}
		}
	}
...
bool BufferedTokenStream::isInitialized() const noexcept
{
	return !_needSetup;
}
...
std::string BufferedTokenStream::getText(gsl::not_null< RuleContext*> ctx )
{
...
void BufferedTokenStream::InitializeInstanceFields() noexcept
{
	_needSetup = true;
	_fetchedEOF = false;
}
