...
namespace antlr4 {
	...
	public:
		BufferedTokenStream( TokenSource* tokenSource ) noexcept;
		...
		std::vector<Token*> get( size_t start, size_t stop );

		size_t LA( ssize_t i ) override;
		Token* LT( ssize_t k ) override;

		/// Reset this token stream by setting its token source.
		void setTokenSource( TokenSource* tokenSource ) noexcept;
		std::vector<Token*> getTokens();
		std::vector<Token*> getTokens( size_t start, size_t stop );
		...
		/// </summary>
		std::vector<Token*> getTokens( size_t start, size_t stop, const std::vector<size_t>& types );
		std::vector<Token*> getTokens( size_t start, size_t stop, size_t ttype );

		/// Collect all tokens on specified channel to the right of
		///  the current token up until we see a token on DEFAULT_TOKEN_CHANNEL or
		///  EOF. If channel is -1, find any non default channel token.
		std::vector<Token*> getHiddenTokensToRight( size_t tokenIndex, ssize_t channel );
		...
		///  or EOF.
		/// </summary>
		std::vector<Token*> getHiddenTokensToRight( size_t tokenIndex );
		...
		/// </summary>
		std::vector<Token*> getHiddenTokensToLeft( size_t tokenIndex, ssize_t channel );

		/// <summary>
		/// Collect all hidden tokens (any off-default channel) to the left of
		///  the current token up until we see a token on DEFAULT_TOKEN_CHANNEL.
		/// </summary>
		std::vector<Token*> getHiddenTokensToLeft( size_t tokenIndex );

		std::string getSourceName() const override;
		std::string getText() override;
		std::string getText( const misc::Interval& interval ) override;
		std::string getText( gsl::not_null<RuleContext*>ctx ) override;
	...
	protected:
		 ...
		 * EOF.
		 */
		ssize_t nextTokenOnChannel( size_t i, size_t channel );
		 ...
		 */
		ssize_t previousTokenOnChannel( size_t i, size_t channel );

		std::vector<Token*> filterForChannel( size_t from, size_t to, ssize_t channel );
